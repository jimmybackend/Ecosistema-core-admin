...
$folder=null; if(isset($options['folder'])){ $f=(string)$options['folder']; if(ctype_digit($f)){ $s=$pdo->prepare('SELECT id,prefix FROM cloud_folders WHERE tenant_id=:t AND user_id=:u AND id=:id AND is_deleted=0 LIMIT 1');$s->execute([':t'=>$tenant,':u'=>$user,':id'=>(int)$f]); } else { $s=$pdo->prepare('SELECT id,prefix FROM cloud_folders WHERE tenant_id=:t AND user_id=:u AND name=:n AND is_deleted=0 ORDER BY id DESC LIMIT 1');$s->execute([':t'=>$tenant,':u'=>$user,':n'=>$f]); } $row=$s->fetch(PDO::FETCH_ASSOC); if(is_array($row)){$folder=$row;}}
