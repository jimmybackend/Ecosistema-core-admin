...
final readonly class MailSmtpAccountRepository
{
    ...
    {
        $stmt = $this->pdo->prepare('SELECT s.* FROM mail_smtp_accounts s INNER JOIN mail_mailboxes m ON m.id = s.mailbox_id WHERE s.tenant_id = :tenant_id AND s.mailbox_id = :mailbox_id AND s.status = :status AND m.tenant_id = :tenant_id AND m.status = :status AND (m.user_id = :user_id OR s.available_to_everyone = 1 OR m.available_to_everyone = 1) ORDER BY s.id DESC LIMIT 1');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT s.* FROM mail_smtp_accounts s LEFT JOIN mail_mailboxes m ON m.id = s.mailbox_id WHERE s.tenant_id = :tenant_id AND s.status = :status AND (m.user_id = :user_id OR s.available_to_everyone = 1 OR m.available_to_everyone = 1) ORDER BY s.available_to_everyone DESC, s.id DESC LIMIT 1');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT s.id, s.name, s.email_address, s.host_in, s.port_in, s.ssl_in, s.host_out, s.port_out, s.ssl_out, s.username, s.status, s.last_error, s.mailbox_id, s.max_daily_email, s.enable_limit, s.available_to_everyone FROM mail_smtp_accounts s LEFT JOIN mail_mailboxes m ON m.id = s.mailbox_id WHERE s.tenant_id = :tenant_id AND (m.user_id = :user_id OR m.available_to_everyone = 1 OR s.created_by_user_id = :user_id OR s.available_to_everyone = 1) ORDER BY s.id DESC LIMIT 100');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT s.* FROM mail_smtp_accounts s LEFT JOIN mail_mailboxes m ON m.id = s.mailbox_id WHERE s.id = :id AND s.tenant_id = :tenant_id AND (m.user_id = :user_id OR m.available_to_everyone = 1 OR s.created_by_user_id = :user_id OR s.available_to_everyone = 1) LIMIT 1');
