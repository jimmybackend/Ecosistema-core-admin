...
'GET /l/{slug}' => static function (array $config, array $params): void {
    ...
    'GET /mail/smtp-accounts/{id}/test-dry-run' => static function (array $config, array $params): void {
        ...
        try { $pdo=PdoFactory::make($config['database']); $repo=new MailSmtpAccountRepository($pdo); $a=$repo->findForUserOrTenant($tenantId,$userId,$id); if(!is_array($a)){ header('Location: /mail/smtp-accounts?error='.urlencode('Cuenta SMTP no encontrada.')); return; }
            $preview=['source'=>'mailbox_smtp','host_in'=>(string)($a['host_in']??''),'port_in'=>(int)($a['port_in']??0),'ssl_in'=>(string)($a['ssl_in']??''),'host_out'=>(string)($a['host_out']??''),'port_out'=>(int)($a['port_out']??0),'ssl_out'=>(string)($a['ssl_out']??''),'username_masked'=>substr((string)($a['username']??''),0,1).'***','email_address'=>(string)($a['email_address']??''),'status'=>(string)($a['status']??''),'password_encrypted_present'=>trim((string)($a['password_encrypted']??''))!==''?'yes':'no'];
