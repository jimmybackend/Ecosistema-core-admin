...
<?php if ($attachments===[]): ?><p>Sin adjuntos pendientes.</p><?php else: ?><table class="eco-table"><thead><tr><th>Archivo</th><th>MIME</th><th>Tamaño</th><th>Estado</th><th>Cloud</th><th>Fecha</th></tr></thead><tbody><?php foreach($attachments as $a): ?><tr><td><?= e((string)($a['original_filename'] ?? '')) ?></td><td><?= e((string)($a['mime_type'] ?? '')) ?></td><td><?= e((string)($a['size_bytes'] ?? '')) ?></td><td><?= e((string)($a['import_status'] ?? '')) ?><?php if (($a['import_status'] ?? '')!=='imported'): ?> · Adjunto pendiente de importar a Cloud<?php endif; ?><?php if (trim((string)($a['error_message'] ?? '')) !== ''): ?><br><small><?= e((string)$a['error_message']) ?></small><?php endif; ?></td><td><?php if ((int)($a['cloud_file_id'] ?? 0) > 0): ?><a href="/cloud/files/<?= (int)$a['cloud_file_id'] ?>">Abrir</a><?php else: ?>-<?php endif; ?></td><td><?= e((string)($a['created_at'] ?? '')) ?></td></tr><?php endforeach; ?></tbody></table><?php endif; ?>
</section></section>
