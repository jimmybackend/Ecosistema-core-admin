...
| Rate limit enforcement | `ECOSISTEMA_RATE_LIMIT_ENABLED`, `ECOSISTEMA_RATE_LIMIT_DRY_RUN`, `ECOSISTEMA_RATE_LIMIT_WRITE_BLOCKS` | `false` | Sí para persistir bloqueos (`WRITE_BLOCKS`) | `permissions.view` en rutas `/security/rate-limit/*` | Sí en POST administrativo (`dry-run`, `enforce`) | Sí (tenant y user desde `AuthSession`) | IPs bloqueadas, patrones de abuso, metadata de intentos | Bloqueos agresivos (falsos positivos) o protección insuficiente (falsos negativos) |
