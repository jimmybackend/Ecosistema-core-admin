...
$pdo=PdoFactory::make($config['database']??[]); $smtp=(new MailSmtpAccountRepository($pdo))->findForUser($tenantId,$userId,$accountId); if(!is_array($smtp)){echo "ACCOUNT_NOT_AUTHORIZED\n"; exit(2);} $mailboxId=(int)($smtp['mailbox_id']??0);
