...
final class PermissionRepository
{
    ...
    public function listRolePermissionIds(int $roleId): array { $stmt=$this->pdo->prepare('SELECT permission_id FROM core_role_permissions WHERE role_id=:role_id');$stmt->execute([':role_id'=>$roleId]);return array_map(static fn($v):int=>(int)$v,$stmt->fetchAll(PDO::FETCH_COLUMN)?:[]); }
    ...
    public function replaceRolePermissions(int $roleId,array $ids): void { $this->pdo->beginTransaction(); try{ $this->pdo->prepare('DELETE FROM core_role_permissions WHERE role_id=:role_id')->execute([':role_id'=>$roleId]); if($ids!==[]){ $ins=$this->pdo->prepare('INSERT INTO core_role_permissions (role_id, permission_id, created_at) VALUES (:role_id, :permission_id, NOW())'); foreach($ids as $pid){$ins->execute([':role_id'=>$roleId,':permission_id'=>$pid]);}} $this->pdo->commit(); }catch(\Throwable $e){ $this->pdo->rollBack(); throw $e; } }
