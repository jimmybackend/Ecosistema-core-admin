...
$m = $pdo->prepare("SELECT m.*, COALESCE(f.folder_name,'INBOX') AS folder_name FROM mail_messages m LEFT JOIN mail_folders f ON f.id=m.folder_id AND f.tenant_id=m.tenant_id WHERE m.tenant_id=:t AND m.user_id=:u AND m.id=:id LIMIT 1");
...
if ($uid <= 0) {
    ...
    if (preg_match('/imap-uid:(\d+)/', $ext, $mm)) { $uid = (int) $mm[1]; }
}
if ($uid <= 0) { echo json_encode(['ok' => false, 'message_id' => $messageId, 'error' => 'missing_message_uid'], JSON_PRETTY_PRINT) . PHP_EOL; exit(2); }

$svc = new MailImapSyncService($pdo, new SecretBox());
$d = $svc->debugMessageForUser($tenantId, $userId, $accountId, (string) ($msg['folder_name'] ?: 'INBOX'), $uid);
...
foreach ($rows as $row) {
    ...
    if (!is_array($raw)) { $raw = []; }
    $raw['imap_folder'] = (string) ($msg['folder_name'] ?: 'INBOX');
    $raw['imap_uid'] = $uid;
