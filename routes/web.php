...
'GET /l/{slug}' => static function (array $config, array $params): void {
    ...
    'POST /roles' => static function (array $config): void {
        ...
        if (!requirePermission($config, 'roles.manage')) { return; }
        $csrfToken = $_POST['_csrf'] ?? null; if (!ensureValidCsrfToken($config, $csrfToken)) { return; }
        $auth = AuthSession::getAuth(); $sessionTenantId = (int) ($auth['tenant_id'] ?? $auth['auth_tenant_id'] ?? 0); $_POST['tenant_id'] = $sessionTenantId;
        try { $pdo = PdoFactory::make($config['database']); $service = new RoleService(new RoleRepository($pdo)); $message = $service->createRole($_POST); if($message==='Rol creado correctamente.'){ auditLog($pdo,['action'=>'role.created','entity_type'=>'core_roles','tenant_id'=>$sessionTenantId,'new_values'=>['code'=>(string)($_POST['code']??''),'name'=>(string)($_POST['name']??''),'status'=>'derivado (sin columna persistida)']]);}  } catch (\Throwable) { $message = 'No se pudo guardar el rol.'; }
    ...
    'GET /mail' => static function (array $config): void {
        ...
        $statusMessage = isset($_GET['ok']) ? (string) $_GET['ok'] : null; $errorMessage = isset($_GET['error']) ? (string) $_GET['error'] : null; $imported=(int)($_GET['imported'] ?? 0); $skipped=(int)($_GET['skipped'] ?? 0); $attachmentsPending=(int)($_GET['attachments_pending'] ?? 0); $syncErrors=(int)($_GET['errors'] ?? 0);
        try { $pdo=PdoFactory::make($config['database']); $service=new MailService(new MailboxRepository($pdo), new MailMessageRepository($pdo)); $messages=$service->listMessages($tenantId,$userId); $smtpAccounts=(new MailSmtpAccountRepository($pdo))->listForImapSyncForUser($tenantId,$userId);} catch (\Throwable) { $messages=[]; $smtpAccounts=[]; $errorMessage='No se pudo cargar el buzón.'; }
    ...
    'POST /mail/smtp-accounts' => static function (array $config): void {
        ...
        try {
            ...
            if ($mailboxRepo->findActiveForUserById($tenantId,$userId,$mailboxId)===null) { header('Location: /mail/smtp-accounts/create?error='.urlencode('Mailbox inválida o inactiva para el usuario actual.')); return; }
            $encrypted=(new SecretBox())->encrypt($password);
            $smtpRepo->create(['tenant_id'=>$tenantId,'mailbox_id'=>$mailboxId,'created_by_user_id'=>$userId,'name'=>trim((string)($_POST['name']??'')),'email_address'=>trim((string)($_POST['email_address']??'')),'host_in'=>trim((string)($_POST['host_in']??'')),'port_in'=>(int)($_POST['port_in']??993),'ssl_in'=>trim((string)($_POST['ssl_in']??'ssl')),'host_out'=>trim((string)($_POST['host_out']??'')),'port_out'=>(int)($_POST['port_out']??587),'ssl_out'=>trim((string)($_POST['ssl_out']??'tls')),'username'=>trim((string)($_POST['username']??'')),'password_encrypted'=>$encrypted,'max_daily_email'=>(int)($_POST['max_daily_email']??0),'enable_limit'=>isset($_POST['enable_limit'])?1:0,'available_to_everyone'=>isset($_POST['available_to_everyone'])?1:0,'status'=>in_array((string)($_POST['status']??'active'),['active','disabled'],true)?(string)$_POST['status']:'active']);
    ...
    'POST /mail/smtp-accounts/{id}' => static function (array $config, array $params): void {
        ...
        try { $pdo=PdoFactory::make($config['database']); $repo=new MailSmtpAccountRepository($pdo); $account=$repo->findForUserOrTenant($tenantId,$userId,$id); if(!is_array($account)){ header('Location: /mail/smtp-accounts?error='.urlencode('Cuenta SMTP no encontrada.')); return; }
            ...
            $repo->update($tenantId,$id,['name'=>trim((string)($_POST['name']??$account['name']??'')),'email_address'=>trim((string)($_POST['email_address']??$account['email_address']??'')),'host_in'=>trim((string)($_POST['host_in']??$account['host_in']??'')),'port_in'=>(int)($_POST['port_in']??$account['port_in']??993),'ssl_in'=>trim((string)($_POST['ssl_in']??$account['ssl_in']??'ssl')),'host_out'=>trim((string)($_POST['host_out']??$account['host_out']??'')),'port_out'=>(int)($_POST['port_out']??$account['port_out']??587),'ssl_out'=>trim((string)($_POST['ssl_out']??$account['ssl_out']??'tls')),'username'=>trim((string)($_POST['username']??$account['username']??'')),'password_encrypted'=>$encrypted,'max_daily_email'=>(int)($_POST['max_daily_email']??$account['max_daily_email']??0),'enable_limit'=>isset($_POST['enable_limit'])?1:0,'available_to_everyone'=>isset($_POST['available_to_everyone'])?1:0,'status'=>in_array((string)($_POST['status']??$account['status']??'active'),['active','disabled'],true)?(string)($_POST['status']??$account['status']):'active']);
    ...
    'POST /mail/smtp-accounts/{id}/delete' => static function (array $config, array $params): void {
        ...
        $csrfToken = $_POST['_csrf'] ?? null; if (!ensureValidCsrfToken($config, $csrfToken)) { return; }
        $auth = AuthSession::getAuth(); $tenantId=authTenantId($auth); $userId=authUserId($auth); $id=(int)($params['id']??0);
        try { $pdo=PdoFactory::make($config['database']); $repo=new MailSmtpAccountRepository($pdo); $account=$repo->findAuthorizedById($tenantId,$userId,$id); if(!is_array($account)){ header('Location: /mail/smtp-accounts?error='.urlencode('No se encontró la cuenta SMTP o no tienes acceso.')); return; } if((string)($account['status'] ?? '') === 'active'){ header('Location: /mail/smtp-accounts?error='.urlencode('Primero desactiva la cuenta SMTP antes de eliminarla.')); return; } if(!$repo->deleteForUser($tenantId,$userId,$id)){ header('Location: /mail/smtp-accounts?error='.urlencode('No se pudo eliminar físicamente la cuenta SMTP desactivada. Verifica dependencias de datos.')); return; } header('Location: /mail/smtp-accounts?ok='.urlencode('Cuenta SMTP eliminada correctamente.')); return; } catch (\Throwable) { header('Location: /mail/smtp-accounts?error='.urlencode('No se pudo eliminar la cuenta SMTP.')); return; }
    },
    ...
    'GET /system/health' => static function (array $config): void {
        ...
        $statusMessage = isset($_GET['ok']) ? (string)$_GET['ok'] : null; $errorMessage = isset($_GET['error']) ? (string)$_GET['error'] : null;
        try { $pdo=PdoFactory::make($config['database']); $service=new HealthService(new HealthRepository($pdo), new LogRepository($pdo), $pdo); $healthChecks=$service->listHealthChecks((int)(AuthSession::getAuth()['auth_tenant_id']??0)); } catch (\Throwable) { $healthChecks=[]; $errorMessage='No se pudo ejecutar el health check.'; }
    ...
    'POST /system/health/{id}/run' => static function (array $config, array $params): void {
        ...
        try { $pdo=PdoFactory::make($config['database']); $service=new HealthService(new HealthRepository($pdo), new LogRepository($pdo), $pdo); $auth=AuthSession::getAuth(); $message=$service->runHealthCheck($id, isset($auth['auth_tenant_id'])?(int)$auth['auth_tenant_id']:null, isset($auth['auth_user_id'])?(int)$auth['auth_user_id']:null, $_SERVER['REMOTE_ADDR']??null, $_SERVER['HTTP_USER_AGENT']??null);} catch (\Throwable) {}
    ...
    'GET /system/logs' => static function (array $config): void {
        startAuthSession($config); if (!AuthSession::isAuthenticated()) { header('Location: /login'); return; }
        if (!requirePermission($config, 'system.view')) { return; }
        $errorMessage=null; try{$pdo=PdoFactory::make($config['database']); $logs=(new LogRepository($pdo))->listRecent((int)(AuthSession::getAuth()['auth_tenant_id']??0),100);}catch(\Throwable){$logs=[]; $errorMessage='No se pudieron cargar los logs.';}
