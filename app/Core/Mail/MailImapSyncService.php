...
final readonly class MailImapSyncService
{
    ...
    private function findAuthorizedAccount(int $tenantId, int $userId, int $accountId): ?array { $sql = "SELECT 'inbound' AS source_type, ia.id AS source_id, ia.tenant_id, ia.mailbox_id, ia.username AS imap_username, ia.password_encrypted, ia.host AS imap_host, ia.port AS imap_port, ia.encryption AS imap_encryption, ia.status, ia.created_by_user_id, ia.available_to_everyone, m.user_id AS mailbox_user_id, m.full_address AS mailbox_full_address FROM mail_inbound_accounts ia INNER JOIN mail_mailboxes m ON m.id = ia.mailbox_id WHERE ia.id = :id AND ia.tenant_id = :tenant_id AND m.tenant_id = :tenant_id AND ia.status = 'active' AND m.status = 'active' AND (m.user_id = :user_id OR m.available_to_everyone = 1 OR ia.created_by_user_id = :user_id OR ia.available_to_everyone = 1) LIMIT 1"; $stmt = $this->pdo->prepare($sql); $stmt->execute([':id'=>$accountId,':tenant_id'=>$tenantId,':user_id'=>$userId]); $row=$stmt->fetch(PDO::FETCH_ASSOC); if (is_array($row)) { return $row; }
        $sql = "SELECT 'smtp' AS source_type, s.id AS source_id, s.tenant_id, s.mailbox_id, s.username AS imap_username, s.password_encrypted, s.host_in AS imap_host, s.port_in AS imap_port, s.ssl_in AS imap_encryption, s.status, s.created_by_user_id, s.available_to_everyone, m.user_id AS mailbox_user_id, m.full_address AS mailbox_full_address FROM mail_smtp_accounts s INNER JOIN mail_mailboxes m ON m.id = s.mailbox_id WHERE s.id = :id AND s.tenant_id = :tenant_id AND m.tenant_id = :tenant_id AND s.status = 'active' AND m.status = 'active' AND (m.user_id = :user_id OR m.available_to_everyone = 1 OR s.created_by_user_id = :user_id OR s.available_to_everyone = 1) LIMIT 1"; $stmt = $this->pdo->prepare($sql); $stmt->execute([':id'=>$accountId,':tenant_id'=>$tenantId,':user_id'=>$userId]); $row=$stmt->fetch(PDO::FETCH_ASSOC); return is_array($row)?$row:null; }
