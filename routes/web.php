...
'GET /l/{slug}' => static function (array $config, array $params): void {
    ...
    'GET /mail' => static function (array $config): void {
        ...
        try {
            ...
            $smtpAccounts = (new MailSmtpAccountRepository($pdo))->listForImapSyncForUser($tenantId, $userId);
        } catch (\Throwable) {
            $smtpAccounts = [];
        }
    ...
    'POST /mail/smtp-accounts/{id}/delete' => static function (array $config, array $params): void {
        ...
        try {
            $pdo = PdoFactory::make($config['database']);
            $repo = new MailSmtpAccountRepository($pdo);
            if (!$repo->deleteForUser($tenantId, $userId, $id)) {
                header('Location: /mail/smtp-accounts?error=' . urlencode('Cuenta SMTP no encontrada o sin acceso.'));
                return;
            }
            header('Location: /mail/smtp-accounts?ok=' . urlencode('Cuenta SMTP eliminada correctamente.'));
            return;
        } catch (\RuntimeException $e) {
            header('Location: /mail/smtp-accounts?error=' . urlencode($e->getMessage()));
            return;
        } catch (\Throwable) {
            header('Location: /mail/smtp-accounts?error=' . urlencode('No se pudo eliminar la cuenta SMTP.'));
            return;
        }
    },
