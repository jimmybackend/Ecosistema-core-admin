...
final readonly class RoleService
{
    ...
    private function normalizeCommon(array $input): ?array { $code=trim((string)($input['code']??'')); $name=trim((string)($input['name']??'')); $status=trim((string)($input['status']??'')); $isSystem=(string)($input['is_system']??''); $tenantId=(int)($input['tenant_id']??0); if($code===''||$name===''||!preg_match('/^[a-z0-9_-]+$/',$code)){return null;} if(!in_array($status,self::ALLOWED_STATUSES,true)||!in_array($isSystem,['0','1'],true)){return null;} if($tenantId<=0 || !$this->repository->tenantExists($tenantId)){return null;} return [':tenant_id'=>$tenantId,':slug'=>$code,':name'=>$name,':description'=>$this->nullableTrim($input['description']??null),':is_system'=>(int)$isSystem]; }
    private function normalizeSystemRoleUpdate(array $input): ?array { $name=trim((string)($input['name']??'')); $status=trim((string)($input['status']??'')); if($name===''){return null;} return [':name'=>$name,':description'=>$this->nullableTrim($input['description']??null)]; }
