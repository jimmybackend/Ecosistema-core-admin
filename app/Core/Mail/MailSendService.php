...
{
    public function __construct(private MailMessageRepository $messages, private MailAttachmentService $attachments, private MailConfig $mailConfig, private MailOutgoingAttachmentService $outgoingAttachments, private ?MailEffectiveSenderResolver $resolver = null, private ?MailSender $sender = null)
    {
    }
    ...
    public function previewDraftSend(int $tenantId, int $userId, int $messageId): array
    {
        ...
        $mailboxId = isset($message['mailbox_id']) ? (int) $message['mailbox_id'] : null;
        $effective = $this->resolveEffective($tenantId, $userId, $mailboxId);
        $smtp = $effective['safe'];
