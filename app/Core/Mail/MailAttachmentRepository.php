...
final readonly class MailAttachmentRepository
{
    ...
    public function listLogicalByMessageForUser(int $tenantId, int $userId, int $messageId, int $limit = 100): array
    {
        $stmt = $this->pdo->prepare(
            'SELECT id, original_name, mime_type, extension, size_bytes, status, uploaded_at
             FROM cloud_files
    ...
    public function listOutgoingByMessageForUser(int $tenantId, int $userId, int $messageId, int $limit = 100): array
    {
        $stmt = $this->pdo->prepare(
            'SELECT id, original_name, mime_type, extension, size_bytes, status, found_in_s3, s3_key
             FROM cloud_files
             WHERE tenant_id = :tenant_id
               AND user_id = :user_id
               AND origin_table = :origin_table
