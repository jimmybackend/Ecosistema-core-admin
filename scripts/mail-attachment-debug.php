...
$out=[]; foreach($rows as $r){$raw=json_decode((string)($r['raw_payload_json']??''),true); if(!is_array($raw)){$raw=[];} $has=['imap_folder'=>!empty($raw['imap_folder']),'imap_uid'=>!empty($raw['imap_uid']),'imap_part_number'=>!empty($raw['imap_part_number'])]; $missing=[]; foreach($has as $k=>$v){ if(!$v){$missing[]=$k;} } $can=$missing===[]; $imported=((int)($r['cloud_file_id']??0))>0||((string)$r['import_status']==='imported'); $action=$imported?'already_imported':($can?'import_attachment':'backfill_imap_metadata'); $lastError=(string)($r['error_message']??''); if(str_contains($lastError, "Unknown column 'is_active'")){ $lastError='missing_imap_attachment_metadata'; } $out[]=['external_attachment_id'=>(int)$r['id'],'original_filename'=>(string)$r['original_filename'],'mime_type'=>(string)$r['mime_type'],'size_bytes'=>(int)$r['size_bytes'],'import_status'=>(string)$r['import_status'],'has_cloud_file'=>((int)($r['cloud_file_id']??0))>0,'last_error'=>$sanitize($lastError),'raw_payload_has'=>$has,'missing_fields'=>$missing,'can_import_binary'=>$can,'suggested_action'=>$action];}
